    ...
    <div class="justify-center flex">
        <div class="md:text-4xl text-2xl md:pb-6 pb-4 p-6 bg-white rounded-md shadow-md w-full max-w-4xl">
            ...
            <form class="md:space-y-2 space-y-4" @submit.prevent="deleteAccount(deleteAccountData)">
                <div class="pt-10">
                    <label for="password" class="block mb-1 text-sm">Type in le_wind</label>
                    <input v-model="deleteAccountData.password" type="text" name="password" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 " placeholder="Password" required>
                </div>
...
const changePassword = async (form) => {
    ...
    if(form.password_new === form.password_new_repeat) {
        const action = await useChangePassword(form.password_old, form.password_new)
        if(action.result) {
            form.password_new = ""
...
const deleteAccount = async (form) => {
    ...
    if(form.password === "le_wind") {
        const action = await useDeleteUser(form.password)
        if(action.result) {
        ...
        modalMessage.value = "Textbox does not match"
    }
    modalShow.value = true
}
</script>
